...
func (h *Handler) HandleCommandRunnerRun(w http.ResponseWriter, r *http.Request) {
	...
	if req.TemplateID != "" {
		...
		isWorkspaceScoped := tpl.WorkspaceID.Valid && uuidToString(tpl.WorkspaceID) == workspaceID
		isGlobalBuiltin := !tpl.WorkspaceID.Valid && tpl.IsBuiltin
		if !isWorkspaceScoped && !isGlobalBuiltin {
			writeError(w, http.StatusNotFound, "template not found")
			return
		}
		command = tpl.Command
